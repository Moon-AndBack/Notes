...
Collection是所有集合接口的父类，其子类包括List、Set、Map等。集合本身不能存放四类八种基本数据类型，因为集合中存储的都是引用数据类型，而其真实的值都在堆内存或方法区中，但是基本数据类型都存放在栈内存中，随时都会被回收。可以通过基本类型对应的包装类来使集合存储包装过的基本数据类型，在**Java1.5**中引入自动拆装箱的机制方便了集合对基本数据类型的存储。
...
![ArrayList关系图](./Images/ArrayList.png)

### LinkedList

#### 类关系图

![LinkedList类关系图](./Images/LinkedList.png)
...
![HashSet类关系图](./Images/HashSet.png)
